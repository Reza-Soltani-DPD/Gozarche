...
export default function Modal(props: IAppProps) {
  ...
  return (
    <div className={`fixed h-[100vh] w-[100vw] top-0 left-0 z-30 ${!show ?"collapse": ""}`}>
      <div
        className={`absolute h-full w-full  bg-black transition-all duration-500  ${!show?"bg-opacity-0 collapse":"bg-opacity-60"}`}
      ...
      <div className="w-full h-full flex items-center justify-center">
		...
		rounded-lg p-4 shadow-[0_0_4px_0_rgba(0,0,0,0.3)] max-w-[96vw] max-h-[96vh] flex flex-col`}>{children}</div>
      </div>
    </div>
  );
}
