...
export const variationOptionsRouter = router({
  getVariationsBySearchField: adminProcedure
    ...
    getVariationsByIds:adminProcedure.input(z.object({ids:z.array(z.string()),includes:z.optional(z.array(z.string()))})).query(async({ctx,input})=>{
      return await ctx.prisma.variationOption.findMany({where:{id:{in:input.ids} },include:input.includes?input.includes.reduce((o, key) => Object.assign(o, {[key]: true}), {}):undefined})
       })
});
